...
const collectButton = (type, title, content) => `<button class="hover-collect" type="button" aria-label="收藏" title="收藏" data-collect-type="${type}" data-collect-title="${encodeURIComponent(title)}" data-collect-content="${encodeURIComponent(content)}">★</button>`;
...
document.body.insertAdjacentHTML("beforeend", `<dialog id="collectDialog"><section class="favorites-card"><div class="composer-head"><div><p class="eyebrow">CHOOSE A COLLECTION</p><h2>收藏到哪里？</h2></div><button class="icon-button" id="closeCollect" type="button" aria-label="关闭">×</button></div><div class="collection-picker" id="collectionPicker">${folders.map(folder => `<button type="button" data-folder="${escapeHtml(folder)}">${escapeHtml(folder)}</button>`).join("")}</div></section></dialog><button class="selection-collect" id="selectionCollect" type="button" aria-label="收藏选中文字" title="收藏选中文字" hidden>★</button>`);
...
window.userStore.ready.then(({ user, data }) => { isAdmin = user?.role === "admin"; favorites = data.favorites; folders = data.folders.length ? data.folders : ["默认收藏"]; renderAdminActions(); });
