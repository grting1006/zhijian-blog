...
const markup = text => {
  ...
  text.split("\n").forEach(line => {
    ...
    else if (image) output.push(`<figure class="inline-image"><img src="${escapeHtml(image[2])}" alt="${escapeHtml(image[1])}" /><figcaption>${escapeHtml(image[1])}${collectButton("image", image[1], line)}</figcaption></figure>`);
