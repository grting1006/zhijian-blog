...
const markup = text => {
  ...
  text.split("\n").forEach(line => {
    ...
    else if (image) output.push(`<figure class="inline-image"><img src="${escapeHtml(image[2])}" alt="${escapeHtml(image[1])}" /><figcaption>${escapeHtml(image[1])}</figcaption></figure>`);
    else if (line.startsWith("## ")) { const heading = line.slice(3); output.push(`<h3 class="collectable-heading" data-content-key="${contentKey(heading)}">${inlineMarkup(heading)}${collectButton("heading", heading, heading)}</h3>`); }
    else if (line.startsWith("> ")) output.push(`<blockquote>${inlineMarkup(line.slice(2))}</blockquote>`);
    else if (line.trim()) output.push(`<p data-content-key="${contentKey(line)}">${inlineMarkup(line)}</p>`);
  });
  flushList();
  return output.join("");
};
...
if (article) { let pendingFavorite; const picker = document.querySelector("#collectDialog"); refreshCollectStates = () => view.querySelectorAll("[data-collect-type]").forEach(button => { const collected = isCollected(button.dataset.collectType, decodeURIComponent(button.dataset.collectContent)); button.classList.toggle("is-collected", collected); }); const renderPicker = () => { document.querySelector("#collectionPicker").innerHTML = folders.map(folder => `<button type="button" data-folder="${escapeHtml(folder)}">${escapeHtml(folder)}</button>`).join(""); }; const openPicker = (type, title, content) => { if (!window.userStore.requireLogin()) return; pendingFavorite = { type, title, content }; renderPicker(); picker.showModal(); }; const saveFavorite = async folder => { if (!pendingFavorite || favorites.some(item => item.type === pendingFavorite.type && item.content === pendingFavorite.content && item.folder === folder)) return; favorites.push({ id:`${Date.now()}-${Math.random().toString(16).slice(2)}`, ...pendingFavorite, folder, articleId:article.id }); await window.userStore.saveUserData({ favorites, folders, notes:window.userStore.data.notes }); refreshCollectStates(); picker.close(); pendingFavorite = null; }; const removeFavorite = async (type, content) => { favorites = favorites.filter(item => item.type !== type || item.content !== content); await window.userStore.saveUserData({ favorites, folders, notes:window.userStore.data.notes }); refreshCollectStates(); }; document.querySelector("#closeCollect").addEventListener("click", () => picker.close()); document.querySelector("#collectionPicker").addEventListener("click", event => { const button = event.target.closest("[data-folder]"); if (button) saveFavorite(button.dataset.folder); }); view.addEventListener("click", event => { const collect = event.target.closest("[data-collect-type]"); if (!collect) return; event.stopPropagation(); const type = collect.dataset.collectType, content = decodeURIComponent(collect.dataset.collectContent); if (isCollected(type, content)) removeFavorite(type, content); else openPicker(type, decodeURIComponent(collect.dataset.collectTitle), content); }); document.addEventListener("userchange", refreshCollectStates); }
...
window.userStore.ready.then(({ user, data }) => { isAdmin = user?.role === "admin"; favorites = data.favorites; folders = data.folders.length ? data.folders : ["默认收藏"]; renderAdminActions(); refreshCollectStates(); });
